...
var mapsvg = d3.select("#vizChoropleth_top").append("svg")
    .attr("width", mapwidth)
...
function updateChoropleth() {
    ...
    // Choropleth update
    var mapupdt = mapsvg.selectAll(".maplines");
    ...
    // Enter legend
    var maplegendinit = maplegend.enter()
        .append("g")
        .attr("class", "maplegendentry");

    maplegendinit.append("rect")
        .attr("x", 50)
        .attr("y", function(d,i){
            return i*20 + 50;
        })
    ...
    maplegendinit.append("text")
        ...
        .text(function(d,i){
            ...
            var smallfmt = d3.format(".1s");

            if (extent[1] < 10){
                return smallfmt(Math.round(+extent[0])) + " - " + smallfmt(Math.round(+extent[1]));
            }

            else if (extent[0] < 10){
                return smallfmt(Math.round(+extent[0])) + " - " + format(Math.round(+extent[1]));
            }
